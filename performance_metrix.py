...
def recommendation(user_id, movie):
    ...
    # Getting the id of the movie for which the user wants a recommendation
    ind = indices[movie]
    # Getting all the similar cosine scores for that movie
    sim_scores = list(enumerate(cosine_sim[ind]))
    # Sorting the list obtained
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
    # Getting all the IDs of the movies that are related to the movie entered by the user
    movie_id = [i[0] for i in sim_scores]

    print('The Movie You Should Watch Next Are --')
    print('ID ,   Name ,  Average Ratings , Predicted Rating ')

    # Variable to print only top 10 movies
    count = 0
    for id in range(0, len(movie_id)):
        # Ensure that the movie entered by the user does not come in their recommendation
        if (ind != movie_id[id]):
            ...
            # To take only those movies that a particular user has not watched yet
            if (movie_id[id] not in rated_movies):
                # To print only those movies which have an average rating that is more than 3.5
                if (avg_ratings > 3.5):
                    count += 1
                    ...
                    predicted_rating = round(svd.predict(user_id, movie_id[id]).est, 2)

                    # Collect actual and predicted ratings for performance evaluation
                    if ratings_dataset[ratings_dataset['movieId'] == movie_id[id]].shape[0] > 0:
                        actual_rating = round(
                            np.mean(ratings_dataset[ratings_dataset['movieId'] == movie_id[id]]['rating']), 2)
                        actual_ratings.append(actual_rating)
                        predicted_ratings.append(predicted_rating)

                    print(f'{movie_id[id]} , {titles[movie_id[id]]} , {avg_ratings}, {predicted_rating}')
                    result.append([titles[movie_id[id]], str('Predicted Rating'), str(predicted_rating)])
                if (count >= 10):
                    break
    ...
    if actual_ratings and predicted_ratings:
        ...
        plt.scatter(actual_ratings, predicted_ratings, color='blue', label='Predicted vs Actual')
        plt.plot([min(actual_ratings), max(actual_ratings)], [min(actual_ratings), max(actual_ratings)], color='red',
...
result = recommendation(i_d, movie_name)